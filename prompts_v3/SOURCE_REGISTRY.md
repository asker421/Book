# GLOBAL PROMPT PROTOCOL

- `prompts_v3/FAIL_CLOSED_PROTOCOL.md` — master source процесса написания и аудита V3.
- Маркер синхронизации: `<!-- FAIL_CLOSED_PROTOCOL_V3 -->`.
- Требование: идентичный полный протокол должен стоять перед индивидуальной частью каждого из 40 `prompts_v3/episodes/*.md`.
- При расхождении master/embedded prompt получает PROCESS SYNC FAIL.

---

# V3 PROMPT PACK — SOURCE REGISTRY

- branch: `main`
- source commit: `17f48f3e1dc0dc03fba362de8435644672b0d28d`
- base tree: `a900abb5796a463cc9040d7b7f8f73985d12e4c6`
- `AGENTS.md`: **ABSENT на source commit**

## Обязательные активные источники
- `FINAL_CANON_OVERRIDE.md`
- `CHAPTER_BY_CHAPTER_CANON.md`
- `V3_ARCHITECTURE_MAP.md`
- `REVEAL_ORDER_CANON.md`
- `KNOWLEDGE_LEDGER.md`
- `DECISION_MOTIVATION_CANON.md`
- `CHARACTER_MOTIVATION_ANTI_IDIOT_CANON.md`
- `CAUSE_EFFECT_LEDGER.md`
- `TIME_CONTINUITY_CANON.md`
- `ASGAR_STATE_LEDGER.md`
- `EPOCH_CAPABILITY_CANON.md`
- `HARD_SCENE_CANON.md`
- `PHYSICS_CANON.md`
- `LOCATION_STAGING_LEDGER.md`
- `MICRO_STAGING_CANON.md`
- `LANGUAGE_CANON.md`
- `TERMINOLOGY_CANON.md`
- `CHARACTER_HISTORY_CANON.md`
- `CHARACTER_REVEAL_MAP.md`
- `CHARACTER_VOICE_CANON.md`
- `RELATIONSHIP_CANON.md`
- `CONFLICT_ANTAGONISM_CANON.md`
- `ASGAR_FLAW_CANON.md`
- `NARRATIVE_STYLE_CANON.md`
- `CANON_AUTHOR_STYLE_V3.md`
- `PROSE_DIALOGUE_CANON.md`
- `READER_EXPERIENCE_CANON.md`
- `CANON_WRITING_TICS.md`
- `FORESHADOWING_PAYOFF_LEDGER.md`
- `AUTHORING_RULES.md`
- `CHAPTER_AUDIT_PROMPT.md`
- `NOVEL_FINAL_AUDIT_CHECKLIST.md`
- `MASTER_ROMAN_AUDIT.md`
- `CANON_SYNC_MATRIX.md`

## Legacy/reference
- `chapters_v2/` — frozen previous prose / material only;
- `STORY_BIBLE.md`, `CONTINUITY.md`, `CHAPTER_MAP.md` — подчиняются активным специализированным канонам и `FINAL_CANON_OVERRIDE.md`;
- существующий V3-текст не превращает ошибку в канон.

## Source-change rule
Если любой из источников выше изменился, исполнитель конкретного prompt обязан перечитать изменившийся файл и revalidate затронутые разделы задания до прозы.
